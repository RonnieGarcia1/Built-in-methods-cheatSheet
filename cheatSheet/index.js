...
console.log(Math.ceil(Math.random() * 10)+ 10) // adds to the value being multiplied to meet the max value in the range
...
console.log(Object.values({ heads: 10, tails: 9 })); //> [ 10, 9 ];
...
function getHigh(highest){
    const map1 = highest.map(x => x * 1);

    return Math.max(...map1);
    }
    console.log(getHigh([4, "10", 9]));
